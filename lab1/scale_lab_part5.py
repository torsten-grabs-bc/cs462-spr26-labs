# Snapshot after Part 4.5 — Filtered aggregation benchmark (final)
...
# ── 4.5 Filtered aggregation benchmark ───────────────────────────
def arrow_aggregate(path):
    """Read Parquet, filter to West, return total revenue."""
    t = pq.read_table(path, columns=["region", "revenue"])
    mask = pc.equal(t["region"], "West")
    west = t.filter(mask)
    return pc.sum(west["revenue"]).as_py()


def json_aggregate(path):
    """Read JSON, filter to West, return total revenue."""
    with open(path) as f:
        rows = json.load(f)
    return sum(r["revenue"] for r in rows if r["region"] == "West")


print("\nFiltered aggregation: total West-region revenue (1,000,000 rows)")
print("-" * 55)

t, total = time_read(
    "Parquet+Arrow agg",
    lambda: arrow_aggregate("/lab/data/sales_1000000.parquet"),
)
print(f"{'Parquet → Arrow (pruned + filter)':<40} {t:>8.4f} s")

t, json_total = time_read(
    "JSON+Python agg",
    lambda: json_aggregate("/lab/data/sales_1000000.json"),
)
print(f"{'JSON → Python loop':<40} {t:>8.4f} s")

print(f"\nWest-region total revenue: {total:,.2f}")
print("-" * 55)

# Safety check: both paths must agree on the total
assert abs(total - json_total) < 0.01, \
    f"Mismatch! Parquet total={total}, JSON total={json_total}"
print("✓ Safety check passed: Parquet and JSON totals match.")
